...
<h4 align="center">Lightweight, stateless WebSocket relay server for <a href="https://karmagate.com">KarmaGate</a> Bind — real-time collaboration for security teams.</h4>
...
KarmaGate Relay is the server component for **KarmaGate Bind** — real-time collaboration between KarmaGate desktop clients.

The relay is a **dumb pipe**: it routes encrypted WebSocket messages between peers. It never reads, stores, or decrypts session content. All data is end-to-end encrypted (XChaCha20-Poly1305) between clients.
...
- **E2E Encryption**: Relay never sees plaintext. All payloads are XChaCha20-Poly1305 encrypted between clients.
...
| `/ws` | GET (Upgrade) | WebSocket connection. Query params: `room`, `token`, `pubkey` (host only) |
