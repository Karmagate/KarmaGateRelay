...
func TestRoom_AddRemove(t *testing.T) {
	...
	c1 := &Client{peerID: "peer-1", send: make(chan []byte, 10)}
	c2 := &Client{peerID: "peer-2", send: make(chan []byte, 10)}
...
func TestRoom_Broadcast(t *testing.T) {
	...
	c1 := &Client{peerID: "peer-1", send: make(chan []byte, 10)}
	c2 := &Client{peerID: "peer-2", send: make(chan []byte, 10)}
	c3 := &Client{peerID: "peer-3", send: make(chan []byte, 10)}
	...
	room.Broadcast("peer-1", []byte("hello"))
...
func TestRoom_LastActivity(t *testing.T) {
	...
	c := &Client{peerID: "peer-1", send: make(chan []byte, 10)}
	room.Add(c)
